const books = [
  ...
  function nameAndAge() {
    // escreva seu código aqui
   const authorAgeAndName =  books.map((book) => (
    {
      age: book.releaseYear - book.author.birthYear,
      author: book.author.name
    }
    ));
    const authorAgeAndNameSorted = authorAgeAndName.sort((ageA, ageB) => 
    ageA.age > ageB.age ? 1 : -1);
        
    return authorAgeAndNameSorted;
    }
...
function oldBooksOrdered() {
    ...
    const oldBooksSorted = oldBooks.sort((bookA, bookB) => bookA.releaseYear - bookB.releaseYear);
...
function oldBooks(array) {
    // escreva seu código aqui
    return array.map((book) => book.name).reverse();
  }
